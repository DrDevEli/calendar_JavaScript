...
/*const eventsArr = [
    {
        ...
        events:[
            {
                title: "Event 1 Lorem Ipsum is simply dummy text of the printing",
                time: "10:00 AM",
            },
            {
                title:"Event 2",
...
function initCalendar (){
    ...
    for(let i = 1; i <= lastDate; i++){
       //check if events present on current day
       ...
       let event = false;
        ...
        month === new Date().getMonth() 
            ...
            getActiveDay(i);
            updateEvents(i);
            //if event found add event class also
    ...
    //add listner after calendar initialize
    addListner();
}
initCalendar();
...
//add eventlistnner to prev and next 

prev.addEventListener("click", prevMonth);
next.addEventListener("click", nextMonth);
//calendar is ready 
//adding goto date and goto today funcionality
...
if(dateInput.value.length > 7){
    //dont allow more than 7 characthers
...
//creating function to add listner on days after rendered
...
function addListner(){
    ...
    days.forEach((day) =>{
        ...
        //call active from already active day
        getActiveDay(e.target.innerHTML);
        updateEvents(Number(e.target.innerHTML));


        //remove active class from active day

        days.forEach((day) =>{
            day.classList.remove("active");
        });

        //if prev month clicked goto prev month and add active 
...
// showing active day events and date at top 
...
function updateEvents(date) {
    ...
    //save events when uupdate is called
    saveEvents();
 }

 //creating function to add events
...
eventsContainer.addEventListener("click", (e)=>{
    if(e.target.classList.contains("event")) {
        ...
        //get the title of event tham search in array by title and delete
        eventsArr.forEach((event) => {
            if(
                event.day == activeDay &&
                event.month == month + 1 &&
                ...
                if(event.events.length == 0 ) {
                    eventsArr.splice(indexOf(event), 1);
...
//lets store events inlocal storage get from there
